...
export default {
  ...
  mounted() {
    window.onload = () => {
      ...
      if (beforeBtn) {
        beforeBtn.addEventListener('click', this.beforeClick);
      }
      ...
      this.nodes.push({
        id: 1, label: '강림도령', T: this.canvas.height / 2 + 50, B: this.canvas.height / 2 - 50, L: this.canvas.width / 2 - 50, R: this.canvas.width / 2 + 50,
      });
      this.initSetting();
  ...
  methods: {
    ...
    saveClick() {
      ...
      if (imageName != null) {
        ...
        for (i = 0; i < this.commandHistory.length; i++) {
          if (this.commandHistory[i].x > r) r = this.commandHistory[i].x - this.padding.x;
          else if (this.commandHistory[i].x < l) l = this.commandHistory[i].x - this.padding.x;
          if (this.commandHistory[i].y > t) t = this.commandHistory[i].y - this.padding.y;
          else if (this.commandHistory[i].y < b) b = this.commandHistory[i].y - this.padding.y;
        }
        const wholeX = Math.max(r + 100, l - 100) - (clientX / 2);
        const wholeY = Math.max(t + 100, b - 100) - (clientY / 2);
        const ratio = Math.min(clientX / (wholeX * 2), clientY / (wholeY * 2));
        console.log(wholeX, wholeY, ratio);
        // this.padding.x = (clientX / 2) * ratio - clientX / 2;
        // this.padding.y = (clientY / 2) * ratio - clientY / 2;
        if (this.canvasScale > 1) {
          for (i = 1; i < this.canvasScale; i += 0.1) {
            this.ctx[0].scale(0.9, 0.9);
            this.scale *= 0.9;
          }
        } else {
        ...
        this.padding.x = clientX * ratio * this.scale - clientX;
        this.padding.y = clientY * ratio * this.scale - clientY;
        this.ctx[0].scale(ratio, ratio);
        this.reDrawAll();
    ...
    beforeClick() {
      if (this.wordIndex > 0) this.wordIndex -= 4;
    ...
    nextClick() {
      if (this.words.length > this.wordIndex + 4) this.wordIndex += 4;
      const word1 = document.querySelector('#recommend1');
      const word2 = document.querySelector('#recommend2');
      const word3 = document.querySelector('#recommend3');
      const word4 = document.querySelector('#recommend4');
